#!/usr/bin/env bash
#
# rclone_03.2.6.sh - Финальная рабочая версия (2.6)
# ==============================================================
# 
# ОПИСАНИЕ:
...
# ВЕРСИЯ: 2.6 (Сентябрь 2025) - ИСПРАВЛЕНЫ КРИТИЧЕСКИЕ ОШИБКИ ПАРСИНГА
...
# Основные исправления в версии 2.6:
# 
# КРИТИЧЕСКИЕ ИСПРАВЛЕНИЯ:
# - ИСПРАВЛЕНО: Переменная ALL_BACKUP_PROCESSES_SUCCESS вместо BACKUP_SUCCESS для правильного отслеживания
# - ИСПРАВЛЕНО: Функция parse_rclone_stats() теперь возвращает правильно разделенные значения
# - ИСПРАВЛЕНО: Функция format_size() корректно работает с большими числами (TB диапазон)
# - ИСПРАВЛЕНО: Генерация текстовых отчетов правильно отображает отдельные поля статистики  
# - ИСПРАВЛЕНО: main() функция НЕ завершается с exit code 1 если резервное копирование успешно
# - ИСПРАВЛЕНО: xargs НЕ вызывает критическую ошибку при проблемах с отдельными процессами
# 
# УЛУЧШЕНИЯ ОТОБРАЖЕНИЯ:
# - Статистика операций теперь показывает правильные отдельные значения вместо сырых данных
# - Размеры файлов корректно форматируются в человекочитаемом виде (GB, TB и т.д.)
# - Результат выполнения правильно определяется как "success" при успешном резервном копировании
# - Время выполнения и скорость отображаются в правильном формате