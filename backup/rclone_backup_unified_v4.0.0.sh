...
retry_command() {
    ...
    for (( attempt = 1; attempt <= retries; attempt++ )); do
        ...
        # Коды возврата rclone (см. `rclone --help`): 0 успех, 1 синтаксис/
        # использование, 2 неклассифицированная ошибка, 3 директория не
        # найдена, 4 файл не найден, 5 временная ошибка, 6 менее серьёзные
        # ошибки, 7 фатальная ошибка, 8 превышен лимит --max-transfer,
        # 9 успех, но файлы не передавались. Код 3 — РЕАЛЬНАЯ ошибка
        # (например, /ceph недоступен) и должен идти в обычный retry ниже,
        # а не считаться успехом — раньше здесь по ошибке стояло `3)` вместо
        # `9)`, из-за чего сбой CephFS длиннее ретрай-бюджета rclone
        # маскировался под "нет изменений для копирования".
        case "$exit_code" in
            0)
