#!/usr/bin/env bash
#
# rclone_04.2.3.sh - Версия с улучшенной отчетностью (2.3)
# ============================================================================
# 
# ОПИСАНИЕ:
...
# - Детальное логирование в JSON и текстовом формате с парсингом статистики rclone
...
# - Улучшенная система отчетности с детализацией операций
# - Поддержка современных методов безопасности и совместимости
#
# АВТОР: Ведущий инженер Андрей Марьяненко
# ВЕРСИЯ: 2.3 (Сентябрь 2025) - УЛУЧШЕНА СИСТЕМА ОТЧЕТНОСТИ И СТАТИСТИКИ
...
# ============================================================================
# РАЗДЕЛ 12: УЛУЧШЕННАЯ СИСТЕМА ПОДСЧЕТА СТАТИСТИКИ И ПАРСИНГА RCLONE
# ============================================================================

# Функция парсинга статистики из JSON логов rclone
parse_rclone_stats() {
    ...
    if [[ ! -f "$jsonlog_file" ]]; then
        ...
        echo "0 0 0 0 0 0 0 0"  # defaults: transfers checks deletes errors totalBytes bytes elapsedTime speed
        return 0
    fi
    
    # Парсинг финальной статистики из JSON логов rclone
    local final_stats
    if command -v jq >/dev/null 2>&1; then
        # Используем jq для извлечения последней записи со статистикой
        final_stats=$(jq -r 'select(.stats != null) | .stats | [.transfers // 0, .checks // 0, .deletes // 0, .errors // 0, .totalBytes // 0, .bytes // 0, .elapsedTime // 0, .speed // 0] | @csv' "$jsonlog_file" 2>/dev/null | tail -1 | tr -d '"')
    else
        # Резервный метод без jq - извлекаем последнюю строку со stats
        final_stats=$(grep '"stats":' "$jsonlog_file" 2>/dev/null | tail -1 | sed 's/.*"stats":{//' | sed 's/}.*//' | tr ',' ' ')
        if [[ -n "$final_stats" ]]; then
            # Простой парсинг ключевых значений из JSON без jq
            local transfers checks deletes errors totalBytes bytes elapsedTime speed
            transfers=$(echo "$final_stats" | grep -o '"transfers":[0-9]*' | cut -d':' -f2 | head -1 || echo "0")
            checks=$(echo "$final_stats" | grep -o '"checks":[0-9]*' | cut -d':' -f2 | head -1 || echo "0")
            deletes=$(echo "$final_stats" | grep -o '"deletes":[0-9]*' | cut -d':' -f2 | head -1 || echo "0")
            errors=$(echo "$final_stats" | grep -o '"errors":[0-9]*' | cut -d':' -f2 | head -1 || echo "0")
            totalBytes=$(echo "$final_stats" | grep -o '"totalBytes":[0-9]*' | cut -d':' -f2 | head -1 || echo "0")
            bytes=$(echo "$final_stats" | grep -o '"bytes":[0-9]*' | cut -d':' -f2 | head -1 || echo "0")
            elapsedTime=$(echo "$final_stats" | grep -o '"elapsedTime":[0-9.]*' | cut -d':' -f2 | head -1 || echo "0")
            speed=$(echo "$final_stats" | grep -o '"speed":[0-9.]*' | cut -d':' -f2 | head -1 || echo "0")
            final_stats="$transfers,$checks,$deletes,$errors,$totalBytes,$bytes,$elapsedTime,$speed"
        else
            final_stats="0,0,0,0,0,0,0,0"
        fi
    fi
    
    # Если статистика не найдена, возвращаем нули
    if [[ -z "$final_stats" || "$final_stats" == "null" ]]; then
        final_stats="0,0,0,0,0,0,0,0"
    fi
...
# Функция безопасного подсчета количества файлов и общего размера - ИСПРАВЛЕНО
calculate_directory_stats() {
    ...
    # ИСПРАВЛЕНО: правильный подсчет файлов
    if file_count=$(timeout 60 rclone lsf "${base_args[@]}" "$path" 2>/dev/null | wc -l); then
        log DEBUG "Количество файлов в $path: $file_count"
    else
        log WARNING "Не удалось подсчитать количество файлов в: $path (возможно, таймаут)"
        file_count=0
    fi
    
    # ИСПРАВЛЕНО: правильный подсчет размера с обработкой пустых результатов
    if total_size=$(timeout 60 rclone lsf --format s "${base_args[@]}" "$path" 2>/dev/null | awk '{if($1 != "") sum += $1} END {printf "%.0f", sum+0}'); then
        ...
        log WARNING "Не удалось подсчитать размер файлов в: $path (возможно, таймаут)"
        total_size=0
    fi
...
# Функция форматирования размера в человекочитаемом виде - ИСПРАВЛЕНО
format_size() {
    local size_bytes="$1"
    ...
    while (($(echo "$size_float >= 1024" | bc -l 2>/dev/null || echo 0) == 1 && unit_index < ${#units[@]} - 1)); do
        size_float=$(echo "scale=2; $size_float / 1024" | bc -l 2>/dev/null || echo "$size_float")
        ((unit_index++))
    done
...
# ============================================================================
# РАЗДЕЛ 13: УЛУЧШЕННАЯ ГЕНЕРАЦИЯ ОТЧЕТОВ И СВОДКИ
# ============================================================================

# Функция записи итоговой сводки с использованием jq - УЛУЧШЕНО
write_summary_with_jq() {
    ...
    # Парсинг статистики rclone из JSON логов
    local rclone_stats
    ...
    jq -n \
        ...
        '{
            ...
            operation_statistics: {
                ...
                transferred_size_human: ((. + 0) as $bytes | if $bytes == 0 then "0 B" else ($bytes | tostring) end | . as $str | $rclone_bytes | tostring),
                total_size_human: ((. + 0) as $bytes | if $bytes == 0 then "0 B" else ($bytes | tostring) end | . as $str | $rclone_total_bytes | tostring)
            },
            sources: $sources
        }' > "$temp_json"
...
# Функция записи сводки без использования jq - УЛУЧШЕНО
...
# Функция генерации человекочитаемой сводки - УЛУЧШЕНО
...
main() {
    ...
    export -f log log_command retry_command dest_from_src backup_directory cmd_to_string calculate_directory_stats format_size
...
# Основные улучшения в версии 2.3:
# - ИСПРАВЛЕНА система подсчета статистики файлов и размеров
# - ДОБАВЛЕН парсинг JSON логов rclone для извлечения детальной статистики операций
# - УЛУЧШЕНА функция форматирования размеров с корректной обработкой больших чисел
# - ДОБАВЛЕНА детальная информация о скопированных/удаленных объектах в отчеты
# - ИСПРАВЛЕНО отображение результата выполнения (success/failure вместо unknown)
# - ДОБАВЛЕНЫ таймауты для команд rclone lsf для предотвращения зависания
# - УЛУЧШЕНА обработка ошибок при подсчете статистики
# - ДОБАВЛЕНА поддержка bc для математических операций с плавающей точкой