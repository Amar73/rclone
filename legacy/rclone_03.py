...
def check_exclude_file():
    """Проверка файла исключений (опциональный)."""
    logger.info(f"Проверка файла исключений: {EXCLUDE_FILE}")
    if not EXCLUDE_FILE.is_file():
        logger.warning(f"Файл исключений {EXCLUDE_FILE} не найден, продолжаем без --exclude-from")
        return False
...
def backup_dir(dir, rclone_config, use_exclude_file):
    ...
    if use_exclude_file:
        ...
        logger.info("Файл исключений не используется в backup_dir")
    ...
    rclone_flags = [
        ...
        f"--log-file={LOGFILE}", "--log-level=INFO"
    ]
    if use_exclude_file:
        rclone_flags.append(f"--exclude-from={EXCLUDE_FILE}")
    if rclone_config:
...
def perform_backup(rclone_config, use_exclude_file):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
        futures = [executor.submit(backup_dir, dir, rclone_config, use_exclude_file) for dir in SOURCEDIRS]
...
def main():
    ...
    use_exclude_file = check_exclude_file()
    ...
    with open(LOCKFILE, "w") as lock_fd:
        ...
        if perform_backup(rclone_config, use_exclude_file):
