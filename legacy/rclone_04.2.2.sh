#!/usr/bin/env bash
#
# rclone_04.2.2.sh - Окончательно исправленная версия (2.2)
# ==================================================================
# 
# ОПИСАНИЕ:
...
# ВЕРСИЯ: 2.2 (Сентябрь 2025) - ИСПРАВЛЕНА ОШИБКА С WHILE READ
...
# ИСПРАВЛЕНИЕ: Переменная для хранения дескриптора файла блокировки
# НЕ МОЖЕТ быть readonly, так как ей будет присвоено значение позже
LOCK_FD=""
...
cleanup() {
    ...
    # Освобождение блокировки только если дескриптор был установлен
    if [[ -n "$LOCK_FD" ]]; then
        flock -u "$LOCK_FD" 2>/dev/null || true
        log DEBUG "Блокировка освобождена"
        
        # ИСПРАВЛЕНИЕ: Закрытие дескриптора с использованием современного синтаксиса bash 4.1+
...
# ИСПРАВЛЕНИЕ: Современная реализация блокировки с автоматическим выделением дескриптора
# Использует возможности bash 4.1+ для автоматического выделения файлового дескриптора
if ((BASH_VERSINFO[0] > 4 || (BASH_VERSINFO[0] == 4 && BASH_VERSINFO[1] >= 1))); then
    # Современный метод с автоматическим выделением дескриптора
    exec {LOCK_FD}>"$LOCKFILE" || {
        log ERROR "Не удалось создать файл блокировки: $LOCKFILE"
        exit 1
    }
else
    # Совместимый метод для старых версий bash
    LOCK_FD=200
    exec 200>"$LOCKFILE" || {
        log ERROR "Не удалось создать файл блокировки: $LOCKFILE"
        exit 1
...
# ============================================================================
# РАЗДЕЛ 8: ПРОВЕРКА И ВАЛИДАЦИЯ ФАЙЛА ИСКЛЮЧЕНИЙ - ИСПРАВЛЕНО
# ============================================================================
...
validate_exclude_file() {
    ...
    # ИСПРАВЛЕНИЕ: Валидация синтаксиса правил исключения (с правильной обработкой EOF)
    # Используем конструкцию, которая корректно работает с set -e при достижении EOF
    local line_number=0 invalid_lines=()
    local line
    
    # ИСПРАВЛЕНО: добавляем || [[ -n "$line" ]] для обработки последней строки без \n
    # и || true для игнорирования кода возврата read при EOF
    ...
    done < "$EXCLUDE_FILE" || true  # ИСПРАВЛЕНИЕ: игнорируем код возврата при EOF
...
#
# Основные улучшения в версии 2.2:
# - ИСПРАВЛЕНА критическая ошибка с while read loop и set -e
# - Использование конструкции || [[ -n "$line" ]] для обработки файлов без финального \n
# - Добавление || true для игнорирования кода возврата read при EOF
# - Современные методы работы с файловыми дескрипторами bash 4.1+
# - Автоматическое выделение файлового дескриптора
# - Улучшенная совместимость со старыми версиями bash
