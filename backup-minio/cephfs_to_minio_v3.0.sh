#!/usr/bin/env bash
# =================================================================================================
# cephfs_to_minio_v3.0.sh — Синхронизация CephFS ➜ MinIO S3
# Версия: 3.0.0 (Март 2026)
#
# НАЗНАЧЕНИЕ СКРИПТА:
...
#     DRY_RUN=true ./cephfs_to_minio_v3.0.sh
#
#   Боевой запуск:
#     ./cephfs_to_minio_v3.0.sh
#
...
#
# ПРИМЕЧАНИЕ ПО NTP:
#   Флаг rclone --update пропускает файлы, у которых destination новее source.
#   При рассинхронизации часов между сервером и MinIO возможны пропуски файлов.
#   Убедитесь, что на сервере работает chronyd/ntpd и время синхронизировано.
#
# CRON (пример — запуск каждый день в 02:00):
#   0 2 * * * root /opt/scripts/cephfs_to_minio_v3.0.sh >> /var/log/backup-ceph-minio/cron.log 2>&1
#
# СТРУКТУРА БАКЕТОВ В MINIO:
#   minio:nbiks-backup/
#     Reads/                 ← синхронизируется с /ceph/data/nbics/Reads
#     Genomes/               ← синхронизируется с /ceph/data/nbics/Genomes
#     data/bio/nextcloud/... ← синхронизируется с /ceph/data/bio/nextcloud/...
#
#   minio:deleted-backup/
#     2026-03-22/
#       Reads/               ← файлы, удалённые из источника 22 марта 2026
#       ...
#
# =================================================================================================
...
# Требуем bash версии 4.0+ — используются ассоциативные массивы и другие возможности bash 4
...
# Имя самого скрипта (без пути) — используется в сообщениях
readonly SCRIPT_NAME="${0##*/}"

# Директория, откуда запущен скрипт — для возможного включения config-файлов рядом
...
readonly RCLONE_TRANSFERS="${RCLONE_TRANSFERS:-20}"          # параллельных файлов в передаче
readonly RCLONE_CHECKERS="${RCLONE_CHECKERS:-8}"             # потоков проверки существования
...
# -------------------------------------------------------------------------------------------------
# declare -g делает переменные глобальными даже при объявлении внутри функции
...
# Не завершает скрипт — только предупреждение, т.к. старые версии могут работать корректно.
check_rclone_version() {
    local rclone_version

    log INFO "Проверка версии rclone"

    # Извлекаем только числовую часть версии: "rclone v1.65.2 ..." → "1.65.2"
...
# ИСПРАВЛЕНО: обрабатываем все три типа файлов (.log, .jsonl, .summary.txt),
# а не только .log как было в v2.1 — иначе JSONL и сводки накапливаются бесконечно.
rotate_old_logs() {
...
cleanup_resources() {
    ...
    # Освобождаем flock-блокировку
    if [[ -n "$LOCK_FD" ]]; then
        flock -u "$LOCK_FD" 2>/dev/null || true
        log DEBUG "Блокировка освобождена"

        # Закрываем файловый дескриптор (синтаксис bash 4.1+)
...
# При получении сигнала прерывания устанавливаем флаг ошибки и выходим с кодом 130
# (стандартный bash-код для SIGINT: 128 + 2).
handle_signal() {
    local signal=$1
    log WARNING "Получен сигнал $signal — инициируется корректное завершение"
    SYNC_SUCCESS=false
    # exit запустит EXIT trap → cleanup_resources
    exit 130
}
...
setup_locking_and_signals() {
    # Открываем lock-файл в файловый дескриптор.
    # В bash 4.1+ можно использовать автоматическое присвоение номера дескриптора {LOCK_FD}>
    if ((BASH_VERSINFO[0] > 4 || (BASH_VERSINFO[0] == 4 && BASH_VERSINFO[1] >= 1))); then
        exec {LOCK_FD}>"$LOCKFILE" || die "Не удалось создать файл блокировки: $LOCKFILE"
    else
        # Для bash 4.0 — фиксированный дескриптор 200
        LOCK_FD=200
...
check_minio_connectivity() {
    ...
    #   --silent            — без progress bar и сообщений об ошибках в stderr
    #   --insecure          — разрешить self-signed сертификаты (типично для внутреннего MinIO)
    #   --max-time 10       — таймаут соединения + передачи = 10 секунд
    ...
    if [[ "$http_code" =~ ^[0-9]{3}$ ]]; then
        if [[ "$http_code" == "200" ]]; then
            ...
            # curl не смог установить соединение (DNS, TLS, firewall)
            log WARNING "MinIO S3 недоступен через healthcheck (нет соединения с ${MINIO_ENDPOINT})"
        else
            # Нестандартный код — healthcheck может быть отключён, пробуем корень
...
#
# При ошибке выводим расширенную диагностику, чтобы администратор сразу видел причину:
# неверный endpoint, проблема TLS, неверные ключи доступа и т.п.
#
# ТИПИЧНЫЕ ПРИЧИНЫ ОШИБКИ "Remote недоступен":
#   - self-signed TLS сертификат: добавьте в rclone.conf строку:
#       no_check_certificate = true
#   - неверный endpoint: проверьте provider, endpoint, port в rclone.conf
#   - неверные ключи: проверьте access_key_id / secret_access_key
#   - сетевая недоступность: firewall, routing, DNS
check_rclone_remotes() {
    log INFO "Проверка доступности rclone remote'ов"

    # Извлекаем имя remote из формата "remote:bucket" → "remote:"
    local main_remote="${MAIN_BACKUP%%:*}:"
    local delete_remote="${DELETE_BACKUP%%:*}:"

    # Внутренняя функция проверки одного remote
    _check_single_remote() {
        ...
        # --contimeout — таймаут на установку TCP-соединения
        # --timeout    — таймаут на ответ после соединения
        if ! rclone lsd "$remote"                 --config="$RCLONE_CONFIG"                 --contimeout=15s                 --timeout=30s                 >/dev/null 2>"$tmp_err"; then
...
# rclone lsd выводит список поддиректорий — если бакет пуст, команда завершится с ошибкой,
# поэтому используем || true и рclone mkdir для создания.
create_bucket_if_needed() {
    ...
    if rclone lsd "$bucket_path" --config="$RCLONE_CONFIG" >/dev/null 2>&1; then
        ...
        log INFO "Бакет не найден, создаётся: $bucket_path"
...
cleanup_old_deleted_data() {
    ...
    local -a delete_cmd=(
        ...
        --min-age "${DELETE_RETENTION_DAYS}d"   # только файлы старше N дней
    ...
    # --leave-root — не удалять сам корневой бакет
    local -a rmdir_cmd=(
...
#
# ИСПРАВЛЕНО: exit code rclone захватывается напрямую через промежуточную переменную,
# а не через PIPESTATUS — это устраняет гонку между set +e и pipefail.
#
...
retry_rclone_command() {
    ...
    for ((attempt = 1; attempt <= retries; attempt++)); do
        ...
        # ИСПРАВЛЕНО: запускаем rclone и сохраняем его вывод во временный файл,
        # чтобы избежать проблем с захватом exit code через pipe при set -e.
        # Альтернатива pipe с PIPESTATUS ненадёжна при активном pipefail.
        local tmp_output
        tmp_output=$(mktemp)
...
#
# Алгоритм:
#   1. Проверяем доступность исходной директории.
#   2. Вычисляем относительный путь назначения.
#   3. Формируем путь для backup удалённых файлов (с датой).
#   4. Запускаем rclone sync с --backup-dir.
#
...
#
# ИСПРАВЛЕНО: относительный путь теперь вычисляется универсально через strip /ceph/data/,
# а не жёстко от /ceph/data/nbics/ — это корректно обрабатывает все три директории,
# включая /ceph/data/bio/nextcloud/... из другого суффикса.
perform_directory_sync() {
    ...
    # ИСПРАВЛЕНО: убираем префикс /ceph/data/ (а не /ceph/data/nbics/ как было в v2.1).
    # Это работает для всех трёх директорий:
    #   /ceph/data/nbics/Reads              → nbics/Reads
    #   /ceph/data/nbics/Genomes            → nbics/Genomes
    ...
    local -a sync_cmd=(
        ...
        # Прогресс-бар в stdout (полезен при интерактивном запуске)
        --progress

        # Копировать симлинки как файлы (не следовать по ним).
        # ВНИМАНИЕ: если симлинки ссылаются за пределы /ceph — это может привести
        # к попытке чтения файлов вне CephFS. Проверьте наличие внешних симлинков:
        ...
        # Создавать пустые директории в destination (полезно для сохранения структуры)
        --create-empty-src-dirs

        # Использовать MD5/SHA1 checksum вместо размер+время для обнаружения изменений.
        # Надёжнее, но медленнее. Требует дополнительных запросов к S3.
        --checksum
        ...
        # Встроенные повторы rclone (поверх нашей retry_rclone_command)
        --retries="$RCLONE_RETRIES"
        --retries-sleep="$RCLONE_RETRIES_SLEEP"

        # Буфер в памяти на каждый поток передачи
        --buffer-size="$RCLONE_BUFFER_SIZE"

        # Пропускать файлы, у которых destination новее source.
        # ВАЖНО: требует синхронизации времени (NTP) между сервером и MinIO.
        --update
...
# Это необходимо потому, что generate_final_summary вызывается ДО выхода из скрипта,
# и $? внутри функции отражает код последней команды, а не финальный код завершения.
#
# ИСПРАВЛЕНО: добавлен явный параметр exit_code для корректного определения результата.
# Ранее при падении на этапе check_rclone_remotes сводка показывала "SUCCESS", потому что
# SYNC_SUCCESS=true (синхронизация не дошла до этапа 4 и флаг не был сброшен), а
# SYNC_PROCESSED=0 (счётчик тоже не был установлен).
generate_final_summary() {
    ...
    if ((final_exit_code != 0)); then
        # Скрипт завершился с ошибкой — независимо от флагов это FAILURE
...

    {
        ...
        echo "--------------------------------------------------------------------------------"
        for dir in "${SOURCEDIRS_ARRAY[@]}"; do
            # ИСПРАВЛЕНО: используем /ceph/data/ как базовый префикс (единообразно)
...
main() {
    ...
    initialize_logging           # создаём лог-файлы и пишем заголовок
    rotate_old_logs              # удаляем устаревшие логи
    check_required_commands      # проверяем наличие утилит
    check_rclone_version         # предупреждаем о старой версии rclone
    check_rclone_config          # проверяем конфиг rclone
    validate_source_directories  # валидируем пути (безопасность)
    setup_locking_and_signals    # захватываем блокировку, регистрируем трапы

    # ------------------------------------------------------------------
    # ЭТАП 2: Проверка доступности хранилищ
    # ------------------------------------------------------------------
    log INFO "ЭТАП 2: Проверка доступности систем хранения"
    check_cephfs_availability    # монтируем /ceph если нужно, проверяем директории
    check_minio_connectivity     # HTTP-проверка MinIO healthcheck endpoint
    check_rclone_remotes         # проверяем rclone remote'ы

    # ------------------------------------------------------------------
    # ЭТАП 3: Подготовка бакетов MinIO
    # ------------------------------------------------------------------
    log INFO "ЭТАП 3: Подготовка инфраструктуры MinIO S3"
    create_bucket_if_needed "$MAIN_BACKUP"    # основной бакет
    create_bucket_if_needed "$DELETE_BACKUP"  # бакет для удалённых файлов
    cleanup_old_deleted_data                  # чистим устаревшие записи об удалениях
    ...
    # Запоминаем общее количество до начала обработки
    SYNC_PROCESSED=${#SOURCEDIRS_ARRAY[@]}
    ...
    for source_dir in "${SOURCEDIRS_ARRAY[@]}"; do
        ...
        if perform_directory_sync "$source_dir"; then
            ...
            # Продолжаем со следующей директорией
        fi
    done
    ...
    # Определяем финальный результат и код выхода:
    #   0 — полный или частичный успех (хотя бы одна директория синхронизирована)
    #   1 — полный провал (ни одной директории не синхронизировано)
    if ((SYNC_SUCCESSFUL == SYNC_PROCESSED)); then
        ...
        # Возвращаем 0 при частичном успехе — cron не будет слать алерт на каждый запуск
        # при временной недоступности одной из директорий
        return 0
    else
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
    ...
    # Итоговое сообщение (cleanup_resources уже вызван через EXIT trap,
    # это только дополнительная строка для наглядности в логе cron)
    if ((exit_code == 0)); then
