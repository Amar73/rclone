...
#
#  v3.4.0 (текущая):
#    Разбор прогона 2026-04-26 (шёл 9.4 суток, 174 323 ошибки, 5 бакетов FAIL)
#    показал первопричину: весь объём ошибок — "failed to get rate limit token,
#    retry quota exceeded". Скрипт сам перегружал Ceph RGW, исчерпывал retry-квоту
#    AWS SDK и получал rc=1 после многочасовых попыток.
#    - ДОБАВЛЕНО: --tpslimit/--tpslimit-burst (RCLONE_TPSLIMIT) — потолок запросов
#      в секунду. Прямое лекарство от retry quota exceeded.
#    - СНИЖЕНО: PARALLEL 4->2, TRANSFERS 32->8, CHECKERS 16->4,
#      S3_UPLOAD_CONCURRENCY 32->8. Суммарный параллелизм падает с ~192 до ~24.
#    - ДОБАВЛЕНО: SYNC_MODE=fast|checksum. --checksum (HeadObject на каждый объект)
#      теперь только в еженедельном прогоне, ежедневный идёт по размеру+mtime.
#    - ДОБАВЛЕНО: BUCKET_TIMEOUT (по умолчанию 6h) через timeout(1). Раньше
#      отдельные попытки висели до 33 часов и прогон не завершался неделями.
#      rc=124/137 не ретраятся — это упёрлось бы в тот же лимит.
#
#  v3.3.1:
...
#
# СНИЖЕНО в v3.4.0 с 4 до 2. Разбор прогона 2026-04-26 показал 174 323 ошибки
# вида "failed to get rate limit token, retry quota exceeded": суммарный
# параллелизм (PARALLEL x TRANSFERS x CHECKERS) исчерпывал retry-квоту AWS SDK,
# и rclone возвращал rc=1 после многочасовых попыток.
: "${PARALLEL:=2}"
...
# ── Режим сверки файлов (нововведение v3.4.0) ────────────────────────────────
#
# fast     — сравнение по размеру и времени модификации (поведение rclone по
#            умолчанию). Быстро: не требует HeadObject на каждый объект.
# checksum — сравнение по контрольной сумме. Надёжнее, но на больших бакетах
#            даёт лавину HeadObject-запросов и был одной из причин того, что
#            прогон 2026-04-26 длился 9.4 суток и не укладывался ни в какое окно.
#
# Штатная схема: ежедневно fast, раз в неделю checksum (см. cron/таймер).
: "${SYNC_MODE:=fast}"

# Предел запросов к S3 в секунду на один процесс rclone.
# Это прямое лекарство от "retry quota exceeded": ограничиваем нагрузку на
# Ceph RGW на стороне клиента, вместо того чтобы упираться в его отказы и
# сжигать retry-квоту SDK. 0 — без ограничения (не рекомендуется).
: "${RCLONE_TPSLIMIT:=50}"
: "${RCLONE_TPSLIMIT_BURST:=100}"

# Жёсткий таймаут на один бакет. В прогоне 2026-04-26 отдельные попытки висели
# до 33 часов, из-за чего скрипт не завершался неделями. Формат — как у
# timeout(1) в GNU coreutils / FreeBSD (например 6h, 90m).
: "${BUCKET_TIMEOUT:=6h}"
...
# СНИЖЕНО в v3.4.0 с 32 до 8 — см. комментарий к PARALLEL.
: "${RCLONE_TRANSFERS:=8}"

# Количество параллельных чекеров (потоков, проверяющих нужно ли копировать файл).
# СНИЖЕНО в v3.4.0 с 16 до 4 — см. комментарий к PARALLEL.
: "${RCLONE_CHECKERS:=4}"
  ...
  for (( attempt=1; attempt<=retries; attempt++ )); do
    ...
    case $rc in
      ...
      3)
        ...
        ;;
      124|137)
        # timeout(1) вернул 124 (TERM по истечении срока) или 137 (128+KILL).
        # Повторять смысла нет: следующая попытка упрётся в тот же лимит и
        # сожжёт ещё BUCKET_TIMEOUT времени. Выходим сразу.
        log ERROR "Превышен таймаут ${BUCKET_TIMEOUT} на бакет — попытки прекращены (rc=$rc)"
        return "$rc"
        ;;
    esac
...
process_bucket() {
  ...
  local -a flags=(
    ...
    # --tpslimit: потолок запросов к S3 в секунду. Ограничиваем себя сами, чтобы
    #             не выгребать retry-квоту SDK на отказах перегруженного Ceph RGW.
  ...
  # Сверка по контрольным суммам — только в режиме checksum (еженедельный прогон).
  # В режиме fast полагаемся на размер+mtime, что на порядки дешевле по запросам.
  ...
  # timeout(1) снаружи rclone: без него один бакет мог висеть десятками часов
  # (в прогоне 2026-04-26 — до 33ч), и скрипт не завершался неделями.
  # --kill-after даёт rclone шанс закрыться по TERM, затем добивает KILL.
  local -a cmd=( timeout --kill-after=60s "$BUCKET_TIMEOUT"
                 rclone "$OPERATION" "${flags[@]}" "$src" "$dst" )
...
# v3.4.0: дочерние процессы xargs обязаны видеть и новые параметры, иначе
# process_bucket подставит пустые значения в флаги rclone.
export SYNC_MODE RCLONE_TPSLIMIT RCLONE_TPSLIMIT_BURST BUCKET_TIMEOUT
