...
#
#  v3.4.2 (текущая):
#    Прогон v3.4.1 (fast-list off, checkers=8, tpslimit=50) подтвердил, что
#    снятие --fast-list — необходимое, но НЕ достаточное условие: средние бакеты
#    прошли (nbgi-db-test, 255k объектов — OK за 2ч45м, под fast-list падал), но
#    мега-бакеты 200k+ (nbgi-db-dev, db-metagenomics) всё равно упёрлись в 6h.
#    Причина — скорость сверки упиралась в tpslimit=50 (~17 проверок/сек).
#    - ПОВЫШЕНО: RCLONE_TPSLIMIT 50->150, RCLONE_TPSLIMIT_BURST 100->300,
#      RCLONE_CHECKERS 8->16. Замер db-ena при этих значениях: ~72 проверки/сек,
#      rate-limit=0. При такой скорости 330k объектов укладываются в ~76 минут.
#
#  v3.4.1:
#    Первый боевой прогон v3.4.0 (2026-07-20, 33ч, 9 из 24 бакетов упали по
#    таймауту 6h) вскрыл настоящую причину медлительности — НЕ параллелизм и НЕ
#    rate-limit (их было 0), а флаг --fast-list. Замер на db-ena 2026-07-22:
#    с --fast-list rclone за 92 мин не сделал ни одной проверки (однопоточный
#    рекурсивный листинг RGW не завершился); без --fast-list — 83% сверки за
#    11 мин, 0 ошибок, 0 rate-limit.
#    - ДОБАВЛЕНО: RCLONE_FAST_LIST (по умолчанию false) — --fast-list убран из
#      безусловных флагов и включается только явно.
#    - RCLONE_CHECKERS возвращён 4->8: фаза сверки латентностно-ограничена,
#      больше чекеров её ускоряют, а rate-limit при этом не возникает.
#
#  v3.4.0:
...
# --fast-list: ПО УМОЛЧАНИЮ ВЫКЛЮЧЕН (v3.4.1). На этом Ceph RGW рекурсивный
# упорядоченный листинг однопоточный и не укладывается в часы даже на ~60k
# объектов, тогда как без него листинг чередуется со сверкой и идёт в разы
# быстрее (замер db-ena 2026-07-22: 92 мин без единой проверки против 83%
# сверки за 11 мин). Ставить true только если конкретный RGW листит быстро.
...
# ПОВЫШЕНО в v3.4.2 с 50 до 150: прогон v3.4.1 показал, что при 50 tps
# мега-бакеты (200k+ объектов) не укладываются в таймаут 6h — скорость сверки
# упиралась в этот лимит (~17 проверок/сек). Замер db-ena при 150 tps дал
# ~72 проверки/сек ПРИ rate-limit=0, т.е. запас есть и перегрузки не возникает.
: "${RCLONE_TPSLIMIT:=150}"
: "${RCLONE_TPSLIMIT_BURST:=300}"
...
# История: v3.4.0 снизил 16->4 (борьба с rate-limit), v3.4.1 вернул до 8,
# v3.4.2 вернул до 16. Реальная причина медленных прогонов была в --fast-list
# (выключен с v3.4.1), а фаза сверки латентностно-ограничена, поэтому больше
# чекеров прямо её ускоряют. При 16 чекерах + tpslimit 150 замер дал ~72
# проверки/сек без единой ошибки rate-limit.
: "${RCLONE_CHECKERS:=16}"
...
process_bucket() {
  ...
  # --fast-list управляется переменной и ПО УМОЛЧАНИЮ ВЫКЛЮЧЕН (v3.4.1).
  # Замер на бакете db-ena (2026-07-22): с --fast-list rclone за 92 минуты не
  # завершил даже листинг (checks=0), т.к. рекурсивное упорядоченное перечисление
  # на этом Ceph RGW однопоточное и патологически медленное. БЕЗ --fast-list
  # rclone чередует листинг и сверку — те же ~60k объектов проверены на 83% за
  # 11 минут, без ошибок и rate-limit. Именно --fast-list был причиной того, что
  # 9 из 24 бакетов упирались в таймаут в прогоне 2026-07-20.
  # Включать обратно только если конкретный RGW отдаёт рекурсивный листинг быстро.
  [[ "$RCLONE_FAST_LIST"   == "true" ]] && { flags+=(--fast-list); log INFO "--fast-list включён (RCLONE_FAST_LIST=true)"; }
...
export SYNC_MODE RCLONE_TPSLIMIT RCLONE_TPSLIMIT_BURST BUCKET_TIMEOUT RCLONE_FAST_LIST
