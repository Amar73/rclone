...
#
#  v4.1.1 (текущая):
#    Переносит на archminio02 находки прогонов archminio01 (см. cephtominio
#    v3.4.1/v3.4.2): главным тормозом больших синхронизаций был --fast-list —
#    рекурсивный листинг назначения MinIO на этом кластере однопоточный и
#    многочасовой. Directory Reads/kgs на archminio02 (2026-07-20) как раз
#    упирались в 8h таймаут.
#    - ДОБАВЛЕНО: RCLONE_FAST_LIST (по умолчанию false) — --fast-list убран из
#      безусловных флагов.
#    - ПОВЫШЕНО: RCLONE_CHECKERS 4->16, RCLONE_TPSLIMIT 50->150 (burst 100->300):
#      фаза сверки латентностно-ограничена, эти значения на archminio01 дали
#      ~72 проверки/сек без rate-limit.
#
#  v4.1.0:
...
# ПОВЫШЕНО в v4.1.1 с 4 до 16: фаза сверки латентностно-ограничена, больше
# чекеров её ускоряют (подтверждено на archminio01), rate-limit не возникает.
readonly RCLONE_CHECKERS="${RCLONE_CHECKERS:-16}"            # потоков проверки существования

# --fast-list ПО УМОЛЧАНИЮ ВЫКЛЮЧЕН (v4.1.1) — см. комментарий у флага ниже и
# опыт archminio01 (cephtominio v3.4.1): на этом MinIO рекурсивный листинг
# назначения патологически медленный. Ставить true только если он листит быстро.
readonly RCLONE_FAST_LIST="${RCLONE_FAST_LIST:-false}"
...
# ПОВЫШЕНО в v4.1.1 с 50 до 150 (burst 100->300): при 50 tps сверка больших
# директорий упиралась в лимит; 150 tps на archminio01 дал ~72 проверки/сек
# при rate-limit=0.
readonly RCLONE_TPSLIMIT="${RCLONE_TPSLIMIT:-150}"
readonly RCLONE_TPSLIMIT_BURST="${RCLONE_TPSLIMIT_BURST:-300}"
...
perform_directory_sync() {
    ...
    local -a sync_cmd=(
        ...
        # ВНИМАНИЕ: --fast-list добавляется ниже условно (v4.1.1, по умолчанию OFF).
        # На archminio01 --fast-list оказался причиной многочасовых таймаутов: на
        # этом Ceph/MinIO рекурсивный листинг назначения однопоточный и не
        # укладывается в часы на больших бакетах. Здесь источник — CephFS, но
        # назначение то же MinIO, поэтому риск тот же.
    ...
    # --fast-list только если явно включён (v4.1.1, по умолчанию OFF). Без него
    # rclone чередует листинг и сверку и на медленном MinIO идёт в разы быстрее.
    if [[ "$RCLONE_FAST_LIST" == "true" ]]; then
        sync_cmd+=(--fast-list)
        log INFO "--fast-list включён (RCLONE_FAST_LIST=true)"
    fi
