...
perform_directory_sync() {
    ...
    local -a sync_cmd=(
        ...
        # Сравнение файлов по MD5/SHA1 checksum — надёжнее, чем размер+время.
        # FIX v4.0: флаг --update убран. Совместное использование --update и --checksum
        # приводило к молчаливому пропуску изменённых файлов: --update пропускал файл
        # если mtime destination новее source, не проверяя checksum. При любом расхождении
        # часов или аномалии mtime изменённый файл мог не попасть в бекап.
        --checksum
